...
//           const SizedBox(height: 16),
//
//           // ── Read-only: Main Location & District ────────────────────────────
//           Obx(() {
//             final hasMain = controller.mainLocation.value != null;
//             final hasDistrict = controller.district.value != null;
//             if (!hasMain && !hasDistrict) return const SizedBox.shrink();
//             return Column(
//               children: [
//                 if (hasMain) ...[
//                   _buildReadOnlyField(
...
//       decoration: BoxDecoration(
//         // Slightly different background to signal non-editable
//         color: const Color(0xFFF0F0F8),
//         borderRadius: BorderRadius.circular(14),
//         border: Border.all(color: const Color(0xFFDDDDED)),
//       ),
//       child: Row(
//         children: [
...
//             ),
//           ),
//           // Lock icon to visually communicate non-editable
...
class AdminEventUpdatePage extends StatelessWidget {
    ...
    return Form(
      ...
      child: ListView(
        ...
        children: [
          ...
          // ── Read-only: Main Location & District ────────────────────────────
          // These fields are displayed-only and can never be edited.
          // Their original values are preserved in the controller and sent
          // as-is in the update payload.
          Obx(() {
            final hasMain = controller.mainLocation.value != null;
            final hasDistrict = controller.district.value != null;
            if (!hasMain && !hasDistrict) return const SizedBox.shrink();
  ...
  // ─── Read-only display field ───────────────────────────────────────────────
  // Lock icon visually communicates this field cannot be edited.
  Widget _buildReadOnlyField({
    required String label,
    required String value,
    required IconData icon,
