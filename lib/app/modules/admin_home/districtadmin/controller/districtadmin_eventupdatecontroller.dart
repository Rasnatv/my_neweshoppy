...
//     super.onInit();
//     fetchStates();
//     // Districts are NOT fetched on init — they load after state is set
//   }
//
//   @override
//   void onClose() {
...
//
//   // ─── STATE CHANGED ────────────────────────────────
//   /// Call this when the user manually picks a new state from the dropdown.
...
//         final body = jsonDecode(response.body);
//         if (body['status'] == true) {
//           await _prefillFromResponse(body['data']);
//         }
//       } else {
//         AppSnackbar.error(ApiErrorHandler.handleResponse(response));
//       }
...
//
//   // ─── Prefill ──────────────────────────────────────
//   /// Prefills form fields from API data. Also fetches districts for the
//   /// pre-selected state so the district dropdown is populated correctly.
//   Future<void> _prefillFromResponse(Map<String, dynamic> data) async {
...
//     selectedDistrict.value = null; // set AFTER districts are loaded
//
//     // Fetch districts for the pre-selected state, then restore district value
//     if (state.isNotEmpty) {
...
class DistrictAdminUpdateEventController extends GetxController {
  ...
  // ─── Prefill ──────────────────────────────────────
  /// Prefills form fields from API data.
  /// Handles case-insensitive matching for state/district and ensures
  /// states are loaded before trying to set selectedState.
  Future<void> _prefillFromResponse(Map<String, dynamic> data) async {
    eventName.text          = data['event_name']     ?? '';
    eventLocation.text      = data['event_location'] ?? '';
    startDate.value         = data['start_date'];
    ...
    // ── Ensure states list is populated before matching ──
    if (states.isEmpty) {
      await fetchStates();
    }
    ...
    // ── Fetch districts for matched state, then match district ──
    await fetchDistricts(matchedState);
  ...
  Future<void> pickDate(BuildContext context, {required bool isStart}) async {
    ...
    final picked = await showDatePicker(
      ...
      builder    : (context, child) => Theme(
        data : Theme.of(context).copyWith(
          colorScheme: const ColorScheme.light(primary: Color(0xFF084E43)),
        ),
        child: child!,
      ),
    );
