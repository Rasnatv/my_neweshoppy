...
//   //   // ✅ Always clear stale data FIRST — badge goes to 0 immediately
//   //   _allCartItems.clear();
//   //   _grandTotal.value = 0.0;
//   //
//   //   // ✅ No token → stop here silently, no popup
...
//   //       }
//   //       // status != 1 → cart already cleared above, nothing to do
//   //     }
//   //     // ✅ No error snackbar on fetch — silently fail, cart stays empty
//   //   } catch (e) {
//   //     debugPrint('fetchCart error: $e');
//   //     // ✅ Silent — no snackbar popup
//   //   } finally {
//   //     isLoading.value = false;
//   //   }
//   // }
//   Future<void> fetchCart() async {
//     // ❌ REMOVED: Don't clear before fetch — causes badge to flash 0
//     // _allCartItems.clear();
//     // _grandTotal.value = 0.0;
//
...
//               double.tryParse(data['grand_total']?.toString() ?? '0') ?? 0.0;
//         } else {
//           // status != 1 means truly empty cart from server
//           _allCartItems.clear();
//           _grandTotal.value = 0.0;
//         }
//       }
//     } catch (e) {
//       debugPrint('fetchCart error: $e');
//       // ✅ Silent — keep existing data on error, don't wipe cart
//     } finally {
//       isLoading.value = false;
//     }
//   }
...
class Restaurantcartcontroller extends GetxController {
  ...
  List<RestaurantCartModel> get cartItems =>
      currentRestaurantId.value == 0
          ? _allCartItems.toList()
          : _allCartItems
          .where((item) => item.restaurantId == currentRestaurantId.value)
          .toList();
  ...
  // ✅ Call this on login — clears old user data then loads new user cart
  Future<void> onUserLogin() async {
    clearCart();
    await fetchCart();
  }

  // ✅ Call this on logout — wipe everything immediately
  void onUserLogout() {
    clearCart();
    currentRestaurantId.value = 0;
  }
  ...
  Future<void> fetchCart() async {
    // No token → clear and stop silently
    if (_authToken.isEmpty) {
      clearCart();
      return;
    }
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (data['status'] == 1) {
          final List items = data['data'] ?? [];
          // ✅ Only update AFTER successful response — badge never flashes 0
          _allCartItems.value =
              items.map((e) => RestaurantCartModel.fromJson(e)).toList();
          _grandTotal.value =
              double.tryParse(data['grand_total']?.toString() ?? '0') ?? 0.0;
        } else {
          // Server says cart is truly empty
          _allCartItems.clear();
          _grandTotal.value = 0.0;
        }
      }
      // Silent fail on non-200 — keep existing data
    } catch (e) {
      debugPrint('fetchCart error: $e');
      // Silent — keep existing data on network error
    } finally {
      isLoading.value = false;
    }
  }
    ...
    try {
      final response = await http.post(
        ...
        body: jsonEncode({
          ...
          'image': image,
          'price': price,
          if (forceReplace) 'force_replace': true,
        }),
      );
      ...
      if (response.statusCode == 200) {
        ...
        if (data['status'] == 0 &&
            (data['message'] as String? ?? '')
                ...
                .contains('meal type')) {
          final confirmed =
          await _showMealTypeConflictDialog(data['message']);
