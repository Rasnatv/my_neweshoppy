...
class ManageproductController extends GetxController {
  ...
  void onInit() {
    ...
    if (!isMerchant && !isAdmin) {
      ...
      _showError('Access Denied',
          'You need merchant privileges to access this page');
      return;
    }
  ...
  // ── FIX: Navigate to update page and refresh list when returning ──
  void navigateToUpdateProduct(int productId) {
    Get.toNamed(
      '/update-product', // adjust to your actual route name
      arguments: {'product_id': productId},
    )?.then((result) {
      // result == true means the update was successful
      if (result == true) {
  ...
  // If you use Get.to() instead of named routes, use this version:
    ...
    Get.defaultDialog(
      ...
      onConfirm: () async {
        ...
        try {
          ...
          final response = await http.delete(
            Uri.parse(
                'https://rasma.astradevelops.in/e_shoppyy/public/api/products/${product.id}'),
            headers: headers,
          );
          ...
          if (response.statusCode == 200 &&
            ...
            _showError(
                'Error', body['message'] ?? 'Failed to delete product');
          }
        } catch (e) {
          Get.back(); // close loading
          _showError('Error', 'Failed to delete product: $e');
