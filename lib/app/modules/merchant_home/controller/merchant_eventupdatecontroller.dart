...
class EventUpdateController extends GetxController {
  ...
  void _populateForm(HEventModel e) {
    ...
    // Locked fields from API
    state.value = (e.state != null && e.state!.isNotEmpty)
        ? e.state
        : null;
    district.value = (e.district != null && e.district!.isNotEmpty)
        ? e.district
        : null;
    mainLocation.value =
    (e.mainLocation != null && e.mainLocation!.isNotEmpty)
  ...
  Future<void> selectEndDate(BuildContext context) async {
    ...
    if (endDate.value != null) {
      final endOnly = DateTime(
          endDate.value!.year, endDate.value!.month, endDate.value!.day);
      safeInitial =
      endOnly.isBefore(firstDate) ? firstDate : endDate.value!;
    } else {
      safeInitial = firstDate;
    }
  ...
  Future<void> updateEvent() async {
    ...
    try {
      ...
      final payload = <String, dynamic>{
        ...
        'end_time'      : _formatTime(endTime.value!),
        'event_location': locationCtrl.text.trim(),
        // locked fields sent back as-is
        if (state.value != null)        'state'        : state.value!,
        if (district.value != null)     'district'     : district.value!,
        if (mainLocation.value != null) 'main_location': mainLocation.value!,
      };
