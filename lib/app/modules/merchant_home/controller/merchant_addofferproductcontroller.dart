// lib/data/controllers/add_offer_product_controller.dart
...
class AddOfferProductController extends GetxController {
  ...
  // ── Constructor params passed from CreateOfferController ─────────────────
  final int offerId;
  ...
  // Total products added to this offer so far
  ...
  // Generate variants from the configured variant types
  void generateVariantsFromType() {
    ...
    for (var combo in newCombinations) {
      bool exists = variants.any(
              (variant) => _areMapsEqual(variant.attributes, combo));
      if (!exists) {
  ...
  List<Map<String, String>> _generateAllCombinations() {
    ...
    types.map((type) => configuredVariantTypes[type]!).toList();
  ...
  Future<void> saveOfferProduct() async {
    ...
    try {
      ...
      print("Error saving offer product: $e");
      Get.snackbar(
        "Error",
