...
class UpdateAdvertisementPage extends StatelessWidget {
  ...
  // District blue / Area purple — matches the list page badges
  static const Color _districtColor = Color(0xFF1565C0);
  static const Color _areaColor     = Color(0xFF6A1B9A);
    ...
    return Form(
      ...
      child: ListView(
        ...
        children: [
          ...
          // ── Location (read-only) ──────────────────────────
          Obx(() {
            final ad = ctrl.advertisement.value;
            if (ad == null) return const SizedBox.shrink();
            if (!ad.hasDistrict && !ad.hasArea) return const SizedBox.shrink();
  ...
  // ── Location Read-Only Card ───────────────────────────────
  Widget _buildLocationReadOnly(dynamic ad) {
    final bool isDistrict = ad.hasDistrict as bool;
    final String label    = isDistrict ? ad.district! : ad.mainLocation!;
    final String typeText = isDistrict ? 'District' : 'Area';
    final Color color     = isDistrict ? _districtColor : _areaColor;
    final IconData icon   = isDistrict
