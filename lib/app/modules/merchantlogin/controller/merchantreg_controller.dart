...
class MerchantRegController extends GetxController {
  ...
  Future<void> fetchStates() async {
    try {
      ...
      AppSnackbar.error("Failed to load states: $e"); // ✅
    } finally {
      isLoadingStates.value = false;
    }
  }
  ...
  Future<void> fetchDistricts(String state) async {
    try {
      ...
      AppSnackbar.error("Failed to load districts: $e"); // ✅
    } finally {
      isLoadingDistricts.value = false;
    }
  }
  ...
  Future<void> fetchLocations(String state, String district) async {
    try {
      ...
      AppSnackbar.error("Failed to load locations: $e"); // ✅
    } finally {
      isLoadingLocations.value = false;
    }
  }
  ...
  bool validateInputs() {
    ...
    if (shopLat.value == 0.0 || shopLng.value == 0.0) {
      AppSnackbar.error("Please set shop location (use map or current location)"); // ✅
      return false;
    }
    return true;
  }
    ...
    try {
      ...
      if (response.statusCode == 201 || response.statusCode == 200) {
        ...
        if (responseData['status'] == true) {
          ...
          AppSnackbar.success(responseData['message'] ?? "Merchant registered successfully!"); // ✅
          Get.offAll(() => MerchantDashboardPage());
        } else {
          AppSnackbar.error(responseData['message'] ?? "Registration failed"); // ✅
        }
      } else {
        final errorData = jsonDecode(response.body);
        AppSnackbar.error(errorData['message'] ?? "Registration failed. Please try again."); // ✅
      }
    } catch (e) {
