...
class IntegratedOfferController extends GetxController {
  ...
  bool validateForm() {
    ...
    if (offerNameCtrl.text.trim().isEmpty) {
      Get.snackbar("Validation Error", "Offer name is required");
      return false;
    }
  ...
  Future<void> saveOfferProduct() async {
    ...
    try {
      ...
      final requestBody = {
        "offer_name": offerNameCtrl.text.trim(),
        "discount_percentage":
      ...
      print("   - Offer: ${requestBody['offer_name']}");
      print("   - Discount: ${requestBody['discount_percentage']}%");
      print("   - Product: ${requestBody['product_name']}");
      print("   - Category ID: ${requestBody['category_id']}");
      print("   - Variants: ${variantsData.length}");
      ...
      if (response.statusCode == 201 || body['status'] == 1) {
        print("✅ Success!");
        Get.snackbar(
          "Success",
        ...
        Get.snackbar(
          "Error",
          body['message'] ?? "Failed to create offer product",
          backgroundColor: Colors.red,
          colorText: Colors.white,
